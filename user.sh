...
VALIDATE $? "Downloading user application artifact"
...
VALIDATE $? "Extracting user application artifact"
...
cp $SCRIPTS_DIR/user.service /etc/systemd/system/user.service
